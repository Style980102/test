...
struct sock_common {
	...
	volatile unsigned char	skc_state;		//等同于TCP状态
	...
	union {
		//已绑定端口的传输控制块利用该字段插入到与之绑定端口信息结构为头结点
		//的链表中。释放端口时，会从中删除。仅用于基于连接的传输控制块，如TCP。
...
struct proto {
	void			(*close)(struct sock *sk,	//
					long timeout);
	int			(*connect)(struct sock *sk,		//传输层接口初始化接口，在创建套接口是，在inet_create()中被调用
